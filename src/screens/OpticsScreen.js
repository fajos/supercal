...
export default function OpticsScreen() {
  ...
  const handleSolve = () => {
    ...
    setTimeout(() => {
      try {
        ...
        const shareText = `Optics Result (${modeLabels[mode]}):\nInput: ${JSON.stringify(params)}\nResult: ${solverResult.result}\n\nSolved with SuperCalc`;
  ...
  const operations = [
    { id: 'lens', label: 'Lens Formula', symbol: '🔍', formula: '1/f = 1/u + 1/v' },
    { id: 'refraction', label: 'Refraction', symbol: '🌈', formula: 'n₁sin θ₁ = n₂sin θ₂' },
  ];
        ...
        >
          ...
          <InputCard style={isTablet && styles.tabletInputCard}>
            ...
            <Text style={styles.sectionLabel}>
              {mode === 'lens' ? 'Lens Parameters' : 'Refraction Parameters'}
            </Text>

            {mode === 'lens' ? (
              <>
                <Text style={styles.inputLabel}>Focal Length, f (cm):</Text>
                <TextInput 
                  style={styles.input} 
                  value={focalLength} 
                ...
                <Text style={[styles.inputLabel, { marginTop: 12 }]}>Object Distance, u (cm):</Text>
                <TextInput 
                  style={styles.input} 
                  value={objectDistance} 
                  onChangeText={setObjectDistance} 
                  keyboardType="decimal-pad" 
                  placeholder="Enter object distance"
                  placeholderTextColor={colors.textSecondary} 
                />

                <Text style={[styles.inputLabel, { marginTop: 12 }]}>Image Distance, v (cm) − leave blank to solve:</Text>
                <TextInput 
                  style={styles.input} 
                  value={imageDistance} 
                  onChangeText={setImageDistance} 
                  keyboardType="decimal-pad" 
              ...
              <>
                <Text style={styles.inputLabel}>Refractive Index, n₁ (initial medium):</Text>
                <TextInput 
                  style={styles.input} 
                  value={n1} 
                  onChangeText={setN1} 
                  keyboardType="decimal-pad" 
                  placeholder="e.g., 1.0 for air"
                  placeholderTextColor={colors.textSecondary} 
                />

                <Text style={[styles.inputLabel, { marginTop: 12 }]}>Refractive Index, n₂ (final medium):</Text>
                <TextInput 
                  style={styles.input} 
                  value={n2} 
                  onChangeText={setN2} 
                  keyboardType="decimal-pad" 
                  placeholder="e.g., 1.5 for glass"
                  placeholderTextColor={colors.textSecondary} 
                />

                <Text style={[styles.inputLabel, { marginTop: 12 }]}>Angle of Incidence, θ₁ (degrees):</Text>
                <TextInput 
                  style={styles.input} 
                  value={theta1} 
                  onChangeText={setTheta1} 
                  keyboardType="decimal-pad" 
