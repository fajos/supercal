export function solveStatistics(rawData) {
  ...
  // Calculate basic statistics early
  const min = sorted[0];
  ...
  const q2 = calculateQuartile(sorted, 0.5); // Same as median
  ...
  steps.push({
    ...
    content: [
      ...
      { type: 'text', text: '' },
      { type: 'text', text: 'Step 1: Sort the data from lowest to highest' },
      { type: 'result', text: `Sorted: [${sorted.join(', ')}]` },
      { type: 'text', text: '' },
  ...
  steps.push({
    ...
    content: [
      ...
      { type: 'result', text: `x̄ = ${mean.toFixed(4)}` },
      { type: 'text', text: '' },
  ...
  steps.push({
    ...
    content: [
      ...
      { type: 'result', text: `σ² = ${variance.toFixed(4)}` },
      { type: 'text', text: '' },
      { type: 'text', text: '4. Standard Deviation = √(Variance):' },
      { type: 'result', text: `σ = √(${variance.toFixed(4)}) = ${stdDev.toFixed(4)}` },
    ],
  });
  ...
  steps.push({
    ...
    content: [
      ...
      { type: 'result', text: `σ = ${stdDev.toFixed(4)}` },
      { type: 'text', text: '' },
      { type: 'text', text: 'Sample (divide by n−1): For a sample of a larger population' },
      { type: 'result', text: `s = ${sampleStdDev.toFixed(4)}` },
      { type: 'text', text: '' },
  ...
  steps.push({
    ...
    content: [
      ...
      ...(outliers.length > 0 
        ? [
            { type: 'result', text: `⚠️ Outliers detected: ${outliers.join(', ')}` },
  ...
  steps.push({
    ...
    content: [
      ...
      { type: 'result', text: `${min} ───[${q1.toFixed(1)} | ${q2.toFixed(1)} | ${q3.toFixed(1)}]─── ${max}` },
      { type: 'text', text: '' },
  ...
  steps.push({
    ...
    content: [
      ...
      ...(Math.abs(mean - median) < mean * 0.05 
        ? [
            { type: 'result', text: '📊 Approximately SYMMETRIC distribution' },
            { type: 'text', text: 'Values are evenly spread around the center.' },
          ]
        : mean > median 
          ? [
              { type: 'result', text: '📈 RIGHT-SKEWED (Positive Skew)' },
              { type: 'text', text: 'Tail extends to the right. Mean is pulled up by high values.' },
            ]
          : [
              { type: 'result', text: '📉 LEFT-SKEWED (Negative Skew)' },
      ...
      { type: 'result', text: `CV = (${stdDev.toFixed(4)} / ${Math.abs(mean).toFixed(4)}) × 100% = ${((stdDev / Math.abs(mean)) * 100).toFixed(2)}%` },
